...
export async function GET(
  ...
  if (!id) return NextResponse.json({ error: "Ticker id is required" });

  const parseId = Number(id);

  if (Number.isNaN(parseId))
    return NextResponse.json({ error: "Ticker id must be a number" });

  const ticket = tickets.find((ticket) => ticket.id === parseId);

  if (!ticket) return NextResponse.json({ error: "Ticker not found" });
